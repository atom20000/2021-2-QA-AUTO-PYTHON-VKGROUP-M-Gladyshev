...
@pytest.mark.UI
def test_create_segment(temp_dir, logger,segment_name, auto_main_page :DashboardPage):
...
@pytest.mark.UI
def test_remove_segment(temp_dir, logger,segment_name, auto_main_page :DashboardPage):
    segment_page = auto_main_page.go_to_segment()
    segment_page.Create_segment(segment_name)
    segment_page.Remove_segment(segment_name)
...
def test_create_campaign(temp_dir, logger, photo_dir, auto_main_page :DashboardPage):
    ...
    campaign_name = f'test_create_campaign{time.time()}'
