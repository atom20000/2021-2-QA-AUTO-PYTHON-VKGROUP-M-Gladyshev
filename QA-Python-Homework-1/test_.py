...
#Тест по проверке изменения контактной информации
    ...
    def test_contact(self,login,set_contact_inf):
        ...
        self.send_key(Contact_Form.FIO_FIELD,set_contact_inf['FIO'])
        self.send_key(Contact_Form.PHONE_FIELD,set_contact_inf['PHONE'])
        self.click_elem(Contact_Form.BUTTON_SAVE_CHANGE)
        self.driver.refresh()
        assert self.find_elem(Contact_Form.FIO_FIELD).get_attribute('value') == set_contact_inf['FIO'] 
        assert self.find_elem(Contact_Form.PHONE_FIELD).get_attribute('value') == set_contact_inf['PHONE']
